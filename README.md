...
------------ | ------------ | ------------ |------------
entities | JSON array | Коды | **Необязательный**. Сущности. Массив кодов.
classes  | JSON array | Коды | **Необязательный**. Классы. Массив кодов.
actions  | JSON array | Коды | **Необязательный**. События. Массив кодов.
methods  | JSON array | Коды | **Необязательный**. Методы. Массив кодов.
...
Поле | Тип | Значение | Описание
------------ | ------------ | ------------ |------------
codes | JSON array | Коды (строка) | **Необязательный**. Коды групп координат (мест положений). По умолчанию `default`. Если данные поступают с устройств по UDP то в качестве кода будет выступать серийный номер устройства.
objects  | JSON array | Идентификаторы | **Необязательный**. Объекты. Массив идентификаторов.
