...
**Важно.** Подписки на события привязываются к паре `(session, identity)`: сервер доставляет события именно той WebSocket-сессии, которая создала подписку. Если `<identity>` указан в URL, подписка через `/observer/subscribe` использует его автоматически, **если** поле `identity` в теле запроса не задано явно. Если передать подписку без привязки к identity, события уйдут в подписку с `identity = "main"` и на данное соединение не придут. При необходимости можно явно указать `identity` в теле подписки — оно имеет приоритет.
